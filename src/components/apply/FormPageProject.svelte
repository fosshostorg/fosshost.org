<script lang="ts">
  ...
  const validator = yup.object().shape({
    ...
    description: yup
      .string()
      .required()
      .min(350, "description must be at least 350 characters"),
...
<main in:fade={{ delay: 0, duration: 400 }}>
  ...
  <p>
    We’d like to know more about the project you are working on. Please specify
    the necessary details below.
  </p>
  <Input
    on:blur={() => {
      errors = validate(validator, "name", data.project, errors);
    }}
    error={errors.name}
    label="Project name"
    type="text"
  ...
  <Input
    minCharacters={350}
    on:blur={() => {
      errors = validate(validator, "description", data.project, errors);
    }}
  ...
  <Input
    on:blur={() => {
      errors = validate(validator, "role", data.project, errors);
    }}
    error={errors.role}
    label="What's your role in the project?"
    type="text"
    placeholder="Supervisor, developer, manager, etc."
    bind:value={data.project.role}
  />
  <Input
    on:blur={() => {
      errors = validate(validator, "site", data.project, errors);
    }}
    error={errors.site}
    label="Your website or git address"
    type="text"
    placeholder="https://myfossproject.com"
    bind:value={data.project.site}
  />
  <div class="buttons">
    <button
      class="back"
    ...
    <button
      class="submit"
      on:click={() => {
        errors = pageValidate(validator, data.project, errors);
      }}
    >
